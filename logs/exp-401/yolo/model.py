...
class YoloModel():
    ...
    def yolo_lite(self,input_data):
        ...
        route_6 = x8
        return route_2, route_3, route_4 , route_5, route_6
    
      
        
    def yolov3_lite(self,input_layer, NUM_CLASS):
        # After the input layer enters the Darknet-19 network, we get 2 branches
        route_2, route_3, route_4 , route_5, conv = self.yolo_lite(input_layer)
...
def calc_seg_loss(_labels,_preds):
    ...
    if TRAIN_USE_SEG_BIN:  
        _label = tf.cast(_labels[0]!=80,dtype=tf.float32)[:,:,:,tf.newaxis]
        _pred = _preds[0]#[:,:,:,tf.newaxis]
        bce  = tf.keras.losses.BinaryCrossentropy(reduction=tf.keras.losses.Reduction.NONE)
        bce_loss = bce(_label,_pred)
        bce_loss_mean = tf.math.reduce_mean(bce_loss,axis=[1,2])
            
        seg_std  = tf.math.reduce_std(bce_loss_mean)
        seg_loss = tf.math.reduce_mean(bce_loss_mean)
        
        _loss_dict['seg_loss']= seg_loss
        _loss_dict['seg_std']=seg_std
        _smp_loss_dict['seg'] = bce_loss_mean
        
        _label = _labels[1][:,:,:,tf.newaxis]
        _pred = _preds[1]
        ...
        mse_loss_mean = tf.math.reduce_sum(mse_loss_fg,axis=[1,2])/tf.math.reduce_sum(fg_mask)\
                      + tf.math.reduce_sum(mse_loss_bg,axis=[1,2])/tf.math.reduce_sum(bg_mask)
        rmse_loss_mean = tf.math.sqrt(mse_loss_mean)
        
        dst_std  = tf.math.reduce_std(rmse_loss_mean)
        dst_loss = tf.math.reduce_mean(rmse_loss_mean)
        _loss_dict['dst_loss']= dst_loss
        _loss_dict['dst_std']=dst_std
        _smp_loss_dict['dst'] = rmse_loss_mean
    ...
    total_loss = 0.0 * seg_loss + 1.0 * dst_loss
    
    return _loss_dict, _smp_loss_dict, total_loss
